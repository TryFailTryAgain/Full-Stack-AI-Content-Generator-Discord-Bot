...
async function generateImageViaReplicate_Flux2Klein9bBase({ userInput, imageModel, numberOfImages, trueDimensions, output_format, output_quality, disable_safety_checker, seed, go_fast, guidance, width, height }) {
    const replicate = new Replicate({
        auth: process.env.API_KEY_REPLICATE,
    });

    console.log('\n---Generating image via Replicate Flux 2 Klein 9B Base---');
    ...
    try {
        ...
        // Flux 2 Klein 9B Base doesn't support num_outputs, so we need to run multiple times
        ...
        console.error('Error generating image with Replicate Flux 2 Klein 9B Base:', error);
        throw error;
    }
}

async function generateImageToImageViaReplicate_Flux2Klein9bBase({ images, image, userInput, strength, disable_safety_checker, go_fast, guidance, output_format, output_quality }) {
    let imageBuffer = [];
    const replicate = new Replicate({
        auth: process.env.API_KEY_REPLICATE,
    });
    ...
    console.log('\n---Generating image-2-Image via Replicate Flux 2 Klein 9B Base---');
    ...
    try {
        ...
        const prediction = await replicate.run('black-forest-labs/flux-2-klein-9b-base', { input });
...
async function generateImageEditViaReplicate_Flux2Klein9bBase({ images, image, userInput, aspect_ratio, seed, guidance, output_format, output_quality, disable_safety_checker, go_fast }) {
    const replicate = new Replicate({ auth: process.env.API_KEY_REPLICATE });
    // Support both single image (legacy) and multiple images
    const inputImages = images || (image ? [image] : []);
    
    console.log('\n---Generating image-edit via Replicate Flux 2 Klein 9B Base---');
    console.log('-Prompt:', userInput);
    console.log('-Number of Input Images:', inputImages.length);
    ...
    try {
        ...
        console.error('Error generating image-edit with Replicate Flux 2 Klein 9B Base:', error);
        throw error;
    }
}

module.exports = {
    generateImageViaReplicate_Flux2Klein9bBase,
    generateImageToImageViaReplicate_Flux2Klein9bBase,
    generateImageEditViaReplicate_Flux2Klein9bBase
};
