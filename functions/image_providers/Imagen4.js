...
async function generateImageViaReplicate_Imagen4({ userInput, aspect_ratio, safety_filter_level, output_format }) {
    const replicate = new Replicate({ auth: process.env.API_KEY_REPLICATE });
    console.log('\n---Generating image via Replicate Imagen-4---');
    ...
    try {
        const prediction = await replicate.run('google/imagen-4', { input });
        ...
        console.error('Error generating image with Replicate Imagen-4:', error);
        throw error;
    }
}

module.exports = { generateImageViaReplicate_Imagen4 };
