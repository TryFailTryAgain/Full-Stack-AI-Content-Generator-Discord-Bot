...
async function generateImageViaReplicate_Imagen4Fast({ userInput, aspect_ratio, safety_filter_level, output_format }) {
    const replicate = new Replicate({ auth: process.env.API_KEY_REPLICATE });
    console.log('\n---Generating image via Replicate Imagen-4-Fast---');
    ...
    try {
        const prediction = await replicate.run('google/imagen-4-fast', { input });
        ...
        console.error('Error generating image with Replicate Imagen-4-Fast:', error);
        throw error;
    }
}

module.exports = { generateImageViaReplicate_Imagen4Fast };
